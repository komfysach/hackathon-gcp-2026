# Google Guessing Game — Participant Kit (Windows)
...
You need:
- a GCP project
- [`gcloud` CLI](https://cloud.google.com/sdk/docs/install) (Windows installer)
- PowerShell (built into Windows — open from Start → "Windows PowerShell" or "Terminal")

Run all the commands below in **PowerShell**.
...
#    (open .env in Notepad or VS Code; replace the placeholder line)
...
uv venv --python 3.13
.venv\Scripts\activate                            # re-run in every new terminal
...
- **`Permission denied on resource project your-gcp-project-id`** — you edited `.env.example` instead of `.env`, or never replaced the placeholder. Run `Get-Content .env` (or `type .env`) and check.
...
- **`.venv\Scripts\activate` fails with execution policy error** — run `Set-ExecutionPolicy -Scope CurrentUser RemoteSigned` once, then retry.
